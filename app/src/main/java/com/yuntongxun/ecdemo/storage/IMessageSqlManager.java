...
public class IMessageSqlManager extends AbstractSQLManager {
    ...
    public static void deleteChattingMessage(long sessionId) {
        while (true) {
            ArrayList<ECMessage> iMessageList = IMessageSqlManager
                    .queryIMessageList(sessionId, 50, null);
    ...
    public static void _deleteChattingMessage(long sessionId) {
        ArrayList<ECMessage> iMessageList = IMessageSqlManager
                .queryIMessageList(sessionId, 0, null);
        if (iMessageList != null && !iMessageList.isEmpty()) {
            ArrayList<String> fileList = new ArrayList<String>();
            for (ECMessage detail : iMessageList) {
                if (detail.getType() != ECMessage.Type.TXT) {
                    ECFileMessageBody body = (ECFileMessageBody) detail
                            .getBody();
                    if (!TextUtils.isEmpty(body.getLocalUrl())) {
                        if (body.getLocalUrl().startsWith("THUMBNAIL://")) {
                            ImgInfo imgInfo = ImgInfoSqlManager.getInstance()
                                    .getImgInfo(detail.getMsgId());
                            if (imgInfo != null) {
                                ImgInfoSqlManager.getInstance().delImgInfo(
                                        imgInfo.getMsglocalid());
                                ...
                                fileList.add(FileAccessor.getImagePathName()
                            ...
                            if (start != -1) {
                                ...
                                fileList.add(ImgInfoSqlManager.getInstance()
                                        .getThumbUrlAndDel(thumbnail));
                            }
                        } else {
    ...
    public static void delSingleMsg(String id) {
        ...
        delMessage(id);
        if (msg.getType() != ECMessage.Type.TXT && msg.getType() != Type.CALL) {
            ArrayList<String> fileList = new ArrayList<String>();
            ECFileMessageBody body = (ECFileMessageBody) msg.getBody();
            if (!TextUtils.isEmpty(body.getLocalUrl())) {
                if (body.getLocalUrl().startsWith("THUMBNAIL://")) {
                    ImgInfo imgInfo = ImgInfoSqlManager.getInstance()
                            .getImgInfo(msg.getMsgId());
                    if (imgInfo != null) {
                        ImgInfoSqlManager.getInstance().delImgInfo(
                                imgInfo.getMsglocalid());
                        if (TextUtils.isEmpty(FileAccessor.getImagePathName()
                                + "/" + imgInfo.getThumbImgPath())) {
                            return;
                        }
                        if (!new File(FileAccessor.getImagePathName() + "/"
                        ...
                        && msg.getUserData().indexOf("THUMBNAIL://") != -1) {
    ...
    public static void deleteChattingMessage(String contactId) {
        long sessionId = ConversationSqlManager
                .querySessionIdForBySessionId(contactId);
...
//				return null;
    ...
    public static List<String> getImageMessageIdSession(long session) {
        String sql = "select msgid from "
                ...
                + " and msgType=" + ECMessage.Type.IMAGE.ordinal();
        Cursor cursor = getInstance().sqliteDB().rawQuery(sql, null);
    ...
    public static List<ECMessage> getDowndFailMsg() {
        String sql = "select * from " + DatabaseHelper.TABLES_NAME_IM_MESSAGE
                + " where msgType=" + ECMessage.Type.IMAGE.ordinal()
